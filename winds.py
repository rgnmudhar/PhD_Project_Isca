...
def plots(ds1, ds2, ds3, ds4, labels, fig_name):
    ...
    plt.plot(lat, uz1, color='#2980B9', linestyle='--', label=labels[0])
    plt.plot(lat, uz2, color='#2980B9', label=labels[1])
    plt.plot(lat, uz3, color='k', linestyle='--', label=labels[2])
    plt.plot(lat, uz4, color='k', label=labels[3])
    ...
    plt.legend(loc='upper center' , bbox_to_anchor=(0.5, -0.07),fancybox=False, shadow=True, ncol=3, fontsize='large')
...
if __name__ == '__main__': 
    ...
    plots(ds1, ds2, ds3, ds4, labels, 'PK_eps0+10_zoz13+18_vtx4')
